...
function isCurrentImageComplete() {
  ...
  const hasLesion = LESIONS.some(l => enabled[l.key] && lesions[l.key] === 1);
  return hasLesion;
}
...
async function loadAnnotations() {
  ...
  for (const [imgId, val] of Object.entries(raw)) {
    if (typeof val === 'string') {
      state.annotations[imgId] = { grade: val, lesions: emptyLesions(), confidence: null };
    } else {
...
function toggleLesion(key, checked) {
  ...
  const ann = state.annotations[imageId] || { grade: '', lesions: emptyLesions(), confidence: null };
  const lesions = { ...ann.lesions };
...
function renderTopbar() {
  ...
  // Suivant : désactivé si dernière image OU si annotation incomplète
...
function renderConfidenceButtons(value) {
  document.querySelectorAll('.confidence-btn').forEach(btn => {
    btn.classList.toggle('active', value !== null && parseFloat(btn.dataset.confidence) === value);
  });
}
